...
(function()
{
  ...
  test("512 bit signing key", 6, function()
  {
    ...
    */
    ...
    ok(Utils.verifySignature(publicKey, signature, data), "Correct signature");

    ok(!Utils.verifySignature(publicKey, signature, data + "1"), "Wrong data");
    ok(!Utils.verifySignature(publicKey, signature, data.substr(0, 3)), "Wrong data");
    ok(!Utils.verifySignature(publicKey, signature.substr(0, 5) + "0" + signature.substr(6), data), "Wrong signature");
    ok(!Utils.verifySignature(publicKey.substr(0, 5) + "R" + publicKey.substr(6), signature, data), "Wrong public key");
    ok(!Utils.verifySignature(publicKey.substr(0, 70) + "8" + publicKey.substr(71), signature, data), "Wrong public key");
  });
  ...
  test("Real-life signature", 1, function()
  {
    ...
    ok(Utils.verifySignature(publicKey, signature, data.join("\0")), "Correct signature");
  });
})();
